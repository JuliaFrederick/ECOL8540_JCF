...
#The beginning time step of data is saved in both data and out, because it runs
#without any modificiations to the code in the last for loop.

#### Exercise 1 - Final epidemic size over a 1yr period looks as below
  ##Assume everyone is susceptible and 1/58mill are infectious
  ##Social dist strategy is 12 weeks
  ##Ro=1.8
  ##Mean infectious period is 2.6 days
...
total.epi<- function(T, phi){
  R0 <- 1.8
  N <-  1  				              #population size
  gamma <- 7/2.6  			        #recovery rate (in years)
  beta <- R0*(gamma)/N 	        #transmission rate
  #phi <- 0.3333                 #reducing transmission rate
  D <- 12                       #Number of weeks reduced
  xstart <- c(S=0, I=0, R=0)	  #initial conditions, must sum to one
  xstart[2] <- 1/58000000
  xstart[1] <- 1-xstart[2]
  xstart[3] <- 1-sum(xstart)
  ...
  out3 <- ode(tail(out2,1)[2:4],                    # start at end of last solution
              ...
              method='ode45',
              rtol=1e-7) 
  total.epi<-out3[8] #the last value of R
  
}

varyPhi <- seq(0,0.5,length=100)
varyTvec <- seq(0.25,20,length=100)
data <-outer(varyTvec,varyPhi,Vectorize(total.epi))
require(fields)
image.plot(varyTvec, varyPhi, data, xlab="T", ylab=expression(phi))
