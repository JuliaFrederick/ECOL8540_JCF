...
####Exercise 2 - Plot the 6 state variables
...
####Exercise 3 - Modify model to allow interentions at time T
...
###################################################
levels <- 15      #Different levels of T


###################################################
### code chunk number 14: sensitivity-ebola.rnw:325-326
###################################################
h2 <-250          #Fraction of simulated points to use
...
####Exercise 4 - use the cor functon to calculate the correlation between 2 sets of
####numbers. Write a script to determine which variable is most highly correlated
####with infections prevented
library(stats)
cor(data$outbreak.size, data, use= "pairwise.complete.obs", method="pearson")
##########betaI      betaH      betaF     alpha      gammah     gammai      gammaf
#[1,] 0.1864465 0.01970713 0.09589566 0.1389434 -0.06480203 -0.1437139 0.007663805
###########gammad     theta1     delta1    delta2            z         T outbreak.size
#[1,] -0.09370463 -0.1597912 0.04020943 0.0694808 -0.009822035 0.1781577             1

##betaI has the highest correlation to outbreak.size

####Exercise 5 - Choose an alternative lower or upper bound and re-run the latin hypercube
####analysis. Does this change the results very much? What parameter would you recommend 
####to be most important for further research?
betaI.min <- 1
betaI.max <- 4
params.set <- cbind(
  betaI = lhs[,1]*(betaI.max-betaI.min)+betaI.min,
...
#Increasing the max changed the results to have a higher peak epidemic size. If you change the min
#and max of betaI you change the course of the disease and increase the epidemic size (when you increase)
#the opposite is true when you decrease the max and min
#I would suggest betaI being the most important value to track, however adjusting this value
#isn't as easy in practice for public health officials unless you implement behavioral changes
