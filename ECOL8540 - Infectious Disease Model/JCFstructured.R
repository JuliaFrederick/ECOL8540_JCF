...
####Exercise 1 - Modify the WAIFW matrix to reflect greater assortativity in mixing.
####What effect does this have on the epidemiologicl dynamics?
parmsEx1<-list(beta=matrix(c(9.336, 2.089, 2.086, 2.037, 2.089, 9.336, 2.086, 2.037, 2.086, 2.086,
...
T=c()               #initialize T, a vector to hold times
while(T0<maxTime){  #loop over times
  y=lsoda(y0,c(T0, T0+365),age.model,parmsEx1) #solve diff'l equation for each time
  T=rbind(T, y[2,1])      #store results
  S=rbind(S, y[2,2:5])
  E=rbind(E, y[2,6:9])
  I=rbind(I, y[2,10:13])
...
#When there is greater assortative mixing the dampening effect occurs sooner in the trajectory
#of the outbreak. The proportion of susceptibles also decreases for all age groups, with the 
#biggest decrease affecting the <5 year old group. 